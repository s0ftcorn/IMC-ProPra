...
public class GraphTest
{
    private static double[] sssp(Graph g, int source) {
        ...
        for (int j=0; j<numberOfNodes; j++) {
            SSSP[0][j] = Double.POSITIVE_INFINITY;
        }
        SSSP[0][source] = 0;
        ...
        for (int i=1; i<numberOfNodes; i++) {
            for (int j=0; j<numberOfNodes; j++) {
                ...
                for (int from=0; from<numberOfNodes; from++) {
                    ...
                    // falls die Kosten > 0 sind (also die Kante from -> j existiert)
                    // und SSSP[i-1][from] < unendlich ist (d.h. wir haben schon einen Wert),
                    // berechnen wir den tempMin. tempMin nimmt dann den kleinsten Wert
                    //      von allen möglichen `from` Knoten
                    if (tempCost > 0 && SSSP[i-1][from] < Double.POSITIVE_INFINITY) {
            ...
            if (args.length == 2) {
                ...
                if (allDistance.length <= 20) {
                    ...
                    for (int i=0; i<allDistance.length; i++) {
                        String kosten = (allDistance[i] == Double.POSITIVE_INFINITY) ? "- nicht erreichbar" : "= "+allDistance[i];
